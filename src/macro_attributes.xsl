...
>
    ...
    <xsl:template name="get_macro_attributes" match="/db:book">
      <macro_attributes>
        <xsl:apply-templates select="//db:chapter[@xml:id!='chapter_5']"/>
      </macro_attributes>
    </xsl:template>

    <xsl:template name="chapter_with_macros" match="//db:chapter">
        <xsl:for-each select=".//db:table">
          <xsl:if test="count(db:thead/db:tr/db:th)=3">
              <xsl:call-template name="table_macro_attributes_3_hdr_cols"/>
          </xsl:if>
          <xsl:if test="count(db:thead/db:tr/db:th)=4">
              <xsl:call-template name="table_macro_attributes_4_hdr_cols"/>
          </xsl:if>
        </xsl:for-each>
    </xsl:template>

    <xsl:template name="table_macro_attributes_3_hdr_cols" match="db:table">
      ...
      <xsl:variable name="is_match">
        <xsl:call-template name="ends-with">
          ...
          <xsl:with-param name="t"><xsl:value-of select="'Macro Attributes'"/></xsl:with-param>
        </xsl:call-template>
      </xsl:variable>
            ...
            <xsl:choose>
              <xsl:when test="$hdr_attr_name = 'Attribute Name' and 
                ...
                <xsl:for-each select=".//db:tr">
                  <xsl:call-template name="table_macro_attributes_3_hdr_cols_row"/>
                </xsl:for-each>
              </xsl:when>
              <xsl:otherwise>
    ...
    <xsl:template name="table_macro_attributes_4_hdr_cols" match="db:table">
      <xsl:variable name="caption">
        <xsl:value-of select="db:caption/text()"/>
      </xsl:variable>
      <xsl:variable name="is_match">
          ...
          <items>
            <xsl:choose>
              <xsl:when test="$hdr_attr_name = 'Attribute Name' and 
                              ...
                              $hdr_desc = 'Attribute Description'">
                <xsl:for-each select=".//db:tr">
                  <xsl:call-template name="table_macro_attributes_4_hdr_cols_row"/>
                </xsl:for-each>
              </xsl:when>
              <xsl:otherwise>
                <error>attribute mismatch</error>
   ...
   <xsl:template name="table_macro_attributes_3_hdr_cols_row" match="db:tr">
    ...
    <xsl:template name="table_macro_attributes_4_hdr_cols_row" match="db:tr">
      <xsl:variable name="nc">
        <xsl:value-of select="count(db:td)"/>
      </xsl:variable>
      <!-- test for number of columns -->
